...
SCORE_PARA_COIN = 5                # Moeda aparece após 5 pontos
CHANCE_MACA_SPAWN = 50             # Chance de 1 em 50 frames
CHANCE_POTION_SPAWN = 75           # Chance de 1 em 75 frames
CHANCE_COIN_SPAWN = 30             # Chance de 1 em 30 frames
DURACAO_POWERUP_S = 5
DURACAO_GHOST_S = 5                # Fantasma persegue por 5 segundos
...
class SnakeGame:
    def __init__(self):
        ...
        # Moeda (NOVO)
        try:
            self.imgCoin = cv2.imread(ARQUIVO_COIN, cv2.IMREAD_UNCHANGED)
            self.imgCoin = cv2.resize(self.imgCoin, (TAMANHO_COIN, TAMANHO_COIN))
        ...
        # Fantasmas (NOVO)
        try:
            self.imgGhost1 = cv2.imread(ARQUIVO_GHOST1, cv2.IMREAD_UNCHANGED)
            self.imgGhost1 = cv2.resize(self.imgGhost1, (TAMANHO_GHOST, TAMANHO_GHOST))
        ...
        try:
            self.imgGhost2 = cv2.imread(ARQUIVO_GHOST2, cv2.IMREAD_UNCHANGED)
            self.imgGhost2 = cv2.resize(self.imgGhost2, (TAMANHO_GHOST, TAMANHO_GHOST))
        ...
        self.ghost_pos = (0, 0)
        self.ghost_timer = 0
        self.ghost_direction = 1  # 1 = direita (ghost2), -1 = esquerda (ghost1)
    ...
    def spawnCoin(self):  # NOVO
        self.coin_active = True
        self.coin_timer = time.time()
        self.coin_pos = random.randint(MARGEM_SPAWN, LARGURA_TELA - MARGEM_SPAWN), \
                        random.randint(MARGEM_SPAWN, ALTURA_TELA - MARGEM_SPAWN)

    def spawnGhost(self):  # NOVO
        self.ghost_active = True
        self.ghost_timer = time.time()
        # Spawna o fantasma em uma posição aleatória
        self.ghost_pos = random.randint(MARGEM_SPAWN, LARGURA_TELA - MARGEM_SPAWN), \
                         random.randint(MARGEM_SPAWN, ALTURA_TELA - MARGEM_SPAWN)
